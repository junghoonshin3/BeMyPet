...
## 9. v3 확장 범위 (Figma 261:8359 기반)

- Last Updated: 2026-02-19
- Scope: `전 화면(홈/관심/상세/댓글/신고/로그인/온보딩/설정/차단/하단탭)`
- 정책: `UI-only`, `라이트/다크 동시 완성`, `한국어 카피 유지 리라이트`

### 9.1 비주얼 시스템

- 컬러는 웜톤에서 블루/뉴트럴 체계로 전환한다.
- 라이트 기준:
  - `Primary` `#F8F9FE`
  - `Secondary` `#006FFD`
  - `Surface` `#FFFFFF`
...
- 기본 라운드 `12dp`, 대형 라운드 `16dp`.
- Primary CTA는 고정 높이 `48dp`(시각 높이 40dp 문법 + 터치 영역 44dp 이상).
- 선택 셀은 `Surface/SecondaryContainer`와 얇은 보더로 상태를 구분한다.
- 하단탭은 플로팅 컨테이너형을 유지하되 선택 색상은 블루 계열로 통일한다.

### 9.3 화면 규칙

- 온보딩: 상단 비주얼 + 하단 텍스트 패널 + 도트 + CTA 구조.
- 로그인: Google/Email 탭 구조 유지, Email은 비활성 준비중 상태 유지.
- 탐색/관심/상세: 카드 위계와 간격을 8/12/16/24 스케일로 통일.
- 댓글/신고/차단/설정: 리스트 셀/액션시트/폼 입력 스타일을 동일 문법으로 정규화.

### 9.4 에셋 정책

- Figma 참조 에셋은 로컬 리소스로 저장 후 사용한다.
- 신규 에셋 네이밍: `fg_{screen}_{purpose}_{state}`.
- 장식 에셋은 `contentDescription = null`, 정보성 에셋은 한국어 설명을 제공한다.
